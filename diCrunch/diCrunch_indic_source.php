...
/* move a vowels away from dandas - single space */
...
if ($_SESSION['src'] == "thai" && !empty($_SESSION['transthai'])) // Thai Specific

{

    $text= preg_replace("/a(?<![iu])/","",$text);
    $text = str_replace("ะ","a",$text);
    $text = str_replace("ั","a",$text);
